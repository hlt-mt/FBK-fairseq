...
class TwoPhaseSequenceGenerator(SequenceGenerator):
    def __init__(
        ...
        eos=None,
        auxiliary_lm_model=None,
        auxiliary_lm_weight=None,
        primary_lm_model=None,
        primary_lm_weight=None,
        encoder_avg_outs=None,
        auxiliary_ilm_weight=None,
        primary_ilm_weight=None,
    ):
        ...
        Args:
            ...
            auxiliary_lm_model (~fairseq.models.FairseqModel): language model used to re-score
                auxiliary probabilities
            auxiliary_lm_weight (float, optional): weight used to re-score the probabilities
                generated by the auxiliary decoder with those of the `auxiliary_lm_model`
            primary_lm_model (~fairseq.models.FairseqModel): language model used to re-score
                primary probabilities
            primary_lm_weight (float, optional): weight used to re-score the probabilities
                generated by the primary decoder with those of the `primary_lm_model`
            encoder_avg_outs (string): path to a file containing pre-computed average
                encoder outputs over the training data
            auxiliary_ilm_weight (float, optional): weight used to disentangle the probabilities
                generated by the internal language model of the auxiliary decoder
            primary_ilm_weight (float, optional): weight used to disentangle the probabilities
                generated by the internal language model of the primary decoder
        """
        super().__init__(
            models, tgt_dict,
            beam_size=beam_size,
        ...
        # Language models and internal language model removal settings
        self.auxiliary_lm_model = auxiliary_lm_model
        self.auxiliary_lm_weight = auxiliary_lm_weight
        self.primary_lm_model = primary_lm_model
        self.primary_lm_weight = primary_lm_weight
        if encoder_avg_outs is not None:
            with open(encoder_avg_outs, 'rb') as f:
                self.avg_encoder_outs = torch.from_numpy(np.load(f))
        self.auxiliary_ilm_weight = auxiliary_ilm_weight
        self.primary_ilm_weight = primary_ilm_weight
