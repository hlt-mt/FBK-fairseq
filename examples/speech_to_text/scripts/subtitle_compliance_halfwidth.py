...
_VERSION = "1.3"  # Wrt 1.2 version:  Modified to support half-width character counting
...
def get_character_width(char: str) -> float:
    """
    Returns the display width of a character.
    
    Half-width characters (ASCII, Latin) return 0.5
    Full-width characters (CJK, etc.) return 1.0
    ...
    # F = Fullwidth, W = Wide: these are full-width (1.0)
    # H = Halfwidth, Na = Narrow, N = Neutral, A = Ambiguous: these are half-width (0.5)
    if width in ('F', 'W'):
        return 1.0
    else:
        return 0.5
...
def calculate_string_width(text: str, use_halfwidth: bool = False) -> float:
    """
    Calculates the display width of a string.
    
    Args:
        text: the input string
        use_halfwidth: if True, count half-width chars as 0.5, otherwise count all as 1
    
    Returns:
        The width of the string (float if use_halfwidth=True, int otherwise)
    """
    if not use_halfwidth:
        return len(text)
...
class SubtitleComplianceStats:
    ...
    def clean_content(subtitle, remove_parenthesis_content=False):
        ...
        if remove_parenthesis_content:
            return re.sub(r'\(.*?\)|\[.*?\]', '', subtitle.content)
        return subtitle.content

    @classmethod
    def from_subtitles(
            cls, subtitles: List[srt.Subtitle], 
            remove_parenthesis_content: bool = False,
            use_halfwidth_counting: bool = False):
        """
        Creates the class from a subtitle content.
        
        Args:
            subtitles: list of srt.Subtitle objects
            remove_parenthesis_content: if True, remove content in parentheses
            use_halfwidth_counting: if True, count half-width chars as 0.5
        """
        cps = []
        cpl = []
        lpb = []
        for sub in subtitles:
            ...
            for ln in content.split('\n'):
                # MODIFIED: Use new width calculation function
                line_len = calculate_string_width(ln.strip(), use_halfwidth_counting)
                block_len += line_len
                num_lines += 1
                cpl.append(line_len)
...
def main(args):
    ...
    if args.use_halfwidth_counting:
        print("Using half-width character counting (ASCII/Latin = 0.5, CJK = 1.0)")
    if args.cite:
    ...
    for srt_file in args.srt_file:
        ...
        subtitle_stats = SubtitleComplianceStats.from_subtitles(
            subtitles, 
            args.remove_parenthesis_content,
            args.use_halfwidth_counting)  # NEW PARAMETER
...
if __name__ == '__main__':
    ...
    # NEW ARGUMENT: Half-width counting
    parser.add_argument(
        "--use-halfwidth-counting", default=False, action='store_true',
        help="if set, half-width characters (ASCII/Latin) are counted as 0.5 instead of 1.0, "
             "while full-width characters (CJK) are counted as 1.0. Useful for mixed-script subtitles. "
             "Added in v1.3.")
