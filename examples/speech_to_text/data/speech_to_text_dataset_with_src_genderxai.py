...
class SpeechToTextDatasetWithSrcGenderXai(SpeechToTextDatasetWithSrc):
    ...
    def __getitem__(
            self, index: int) -> Tuple[
                ...
                List[str],
                torch.Tensor]:
        item = super().__getitem__(index)   # contains (index, source, target, transcript)

        # If there is no target and/or transcript
        if len(item) == 2:
            item = item + (None, None)
        elif len(item) == 3:
            item = item + (None,)
        ...
        swapped_tgt_tokens = self.tgt_dict.encode_line(
            tokenized, add_if_not_exist=False, append_eos=True).long()
    ...
    def collater(
        ...
        frames = _collate_frames(
            [s for _, s, _, _, _, _, _, _ in samples], self.data_cfg.use_audio_input
        )
        ...
        if self.tgt_texts is not None:
            target = fairseq_data_utils.collate_tokens(
                [t for _, _, t, _, _, _, _, _ in samples],
                self.tgt_dict.pad(),
                self.tgt_dict.eos(),
                left_pad=False,
            ...
            target_lengths = torch.tensor(
                [t.size(0) for _, _, t, _, _, _, _, _ in samples], dtype=torch.long
            ...
            ntokens = sum(t.size(0) for _, _, t, _, _, _, _, _ in samples)
        ...
        if self.src_texts is not None:
            ...
            ntokens_transcript = sum(t.size(0) for _, _, _, t, _, _, _, _ in samples)

        # Extra fields for gender analysis
        found_terms = [ft for _, _, _, _, ft, _, _, _ in samples]
...
class SpeechToTextDatasetCreatorWithSrcGenderXai(SpeechToTextDatasetCreatorWithSrc):
    ...
    def _from_list(
            ...
            pre_tokenizer,
            bpe_tokenizer,
            bpe_tokenizer_src) -> SpeechToTextDatasetWithSrcGenderXai:
        ...
        return SpeechToTextDatasetWithSrcGenderXai(
            split_name,
            is_train_split,
            data_cfg,
            audio_paths,
