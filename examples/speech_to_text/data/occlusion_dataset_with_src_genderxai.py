...
class OccludedSpeechToTextDatasetWithSrcGenderXai(OccludedSpeechToTextDataset):
    ...
    def __getitem__(self, perturb_index: int) -> Tuple[
        ...
        index, fbank, predicted_tokens, source_text, found_terms, found_term_pairs, \
            gender_terms_indices, swapped_tgt_tokens, *_ = returned_tuple
    ...
    def collater(
            self,
            samples: List[Tuple[
                int, int, Tensor, Tensor, Tensor, Union[Tensor, None], List[str], List[str], List[str], Tensor]]) -> Dict:
        if len(samples) == 0:
            return {}
        ...
        gender_term_starts = [int(gti.split('-')[0]) for _, _, _, _, _, _, _, _, gti, _ in samples]
        gender_term_ends = [int(gti.split('-')[1]) for _, _, _, _, _, _, _, _, gti, _ in samples]
        ...
        for i, (_, _, _, _, tgts, _, _, _, _, swapped_tgts) in enumerate(samples):
