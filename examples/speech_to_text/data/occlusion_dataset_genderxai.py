...
class OccludedSpeechToTextDatasetGenderXai(OccludedSpeechToTextDataset):
    ...
    the gender swapped.
    This class is meant to be used when occluding a dataset without source texts, 
    typically, SpeechToTextDatasetGenderXai.
    """

    def __getitem__(self, perturb_index: int) -> Tuple[
            int, int, Tensor, Tensor, Tensor, List[str], List[str], List[str], Tensor]:
        
        orig_dataset_index = self._original_index(perturb_index)
        returned_tuple = self.dataset[orig_dataset_index]

        index, fbank, predicted_tokens, found_terms, found_term_pairs, \
    ...
    def collater(self, samples: List[Tuple[
        ...
        if len(samples) == 0:
            return {}
        perturb_indices = torch.tensor([i for i, _, _, _, _, _, _, _, _ in samples], dtype=torch.long)
        orig_indices = torch.tensor([i for _, i, _, _, _, _, _, _, _ in samples], dtype=torch.long)
        masks = _collate_frames([m for _, _, m, _, _, _, _, _, _ in samples])
        frames = _collate_frames([s for _, _, _, s, _, _, _, _, _ in samples])
        n_frames = torch.tensor([s.size(0) for _, _, _, s, _, _, _, _, _ in samples], dtype=torch.long)
        ...
        target = data_utils.collate_tokens(
            [p for _, _, _, _, p, _, _, _, _ in samples],
            self.tgt_dict.pad(),
            self.tgt_dict.eos(),
            left_pad=False,
        ...
        target_lengths = torch.tensor(
            [p.size(0) for _, _, _, _, p, _, _, _, _ in samples], dtype=torch.long
        ...
        found_terms = [ft for _, _, _, _, _, ft, _, _, _ in samples]
