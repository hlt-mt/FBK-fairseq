    ...
    with torch.no_grad():
        ...
        for model, enc_out in zip(models, encoder_outs):
            probs.append(model.auxiliary_decoder.get_normalized_probs(
                model.auxiliary_decoder.forward(enc_out)))
        if len(probs) == 1:
            return probs[0]
...
def _main(args, output_file):
    ...
    logger = logging.getLogger('fairseq_cli.generate_multitask_classifier')
    ...
    for sample in progress:
        ...
        for i, sample_id in enumerate(sample['id'].tolist()):
            if not args.quiet:
                print('S-{}\t{}'.format(sample_id, sample['auxiliary_target'][i][0]), file=output_file)

            print('H-{}\t{}\t{}'.format(sample_id, hypos[i].argmax(), hypos[i]), file=output_file)
