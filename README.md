...
| Model | --arch | En-De | En-Nl | En-Es |
|---|---|---|---|---|
| PlainConvattn | speechformer_m | [23.29](https://drive.google.com/file/d/1TiOCkbuAHrFYn7iocGxAqrO6Of7mbK_2/view?usp=sharing) | [27.18](https://drive.google.com/file/d/1SygowBQIKbpiEgkTEeUqa9JOTAuQkR6M/view?usp=sharing) | [28.01](https://drive.google.com/file/d/1iIQr8LeA4MVKbnuLf74zJQf85AwHsofD/view?usp=sharing) |
| Speechformer | speechformer_m | [23.84](https://drive.google.com/file/d/1eoUueH_ZQu6LW4VWUp-hI0IqIY5AXQf2/view?usp=sharing) | [27.85](https://drive.google.com/file/d/1VaVoM8zJSRnsHNNEVYBcbj-ZpHmk7m7h/view?usp=sharing) | [28.56](https://drive.google.com/file/d/1Is9ffeZIufNfpsdnHAWEbaIdIRM1rkRA/view?usp=sharing) |
...
python ${FAIRSEQ_DIR}/examples/speech_to_text/preprocess_generic.py \
  ...
  --src-normalize 
```
where `FAIRSEQ_DIR` is the path to this Fairseq installation, `DATA_ROOT` is the path to the language-specific MuST-C 
directory, and `MUSTC_SAVE_DIR` is the path where you want to save the preprocessed files.
...
Speechformer) is *mandatory* to train successfully the Speechformer architecture.
###PlainConvattention
```sh
...
```
where `ST_SAVE_DIR` is the directory in which you want to save the trained model and `CONFIG_YAML_NAME` is the name of 
the .yaml file. 
`--update-freq` is used to simulate a batch of 40000 tokens and 8 GPUs V-100 32G as in the [Fairseq Speech To Text](https://github.com/pytorch/fairseq/tree/master/examples/speech_to_text) implementation.

### Speechformer
The first step is to select only the first part of the PlainConvattention encoder (until the layer to which the CTC is 
applied), running this script:
```sh
...
Then, to start the training, run the following script:
```sh
fairseq-train ${MUSTC_SAVE_DIR} \
        --train-subset train_st_src --valid-subset dev_st_src \
        --save-dir ${ST_SAVE_DIR} \
        ...
        --skip-invalid-size-inputs-valid-test
```
and you can use the parameter `--patience` to early stopping the training once the loss does not improve in a certain 
number of epochs (15 in our case).

## Generate
For the generate phase, you have first to *average 7 checkpoints*, among which the middle one is the best checkpoint 
obtained during training. 
In your training folder, preserve only the best checkpoint, the 3 before and the 3 after that one. 
...
```
where `AVERAGE_CHECKPOINT_NAME` is the name that you want to give to the average 7 checkpoint.

Then run the following script to perform the generate:
```sh
...
```
Note that we set `--max-tokens 25000` since we use only one GPU K-80 to perform the generate.
