...
class TestAccumulator(unittest.TestCase):
    ...
    def test_decode_line_dataset_with_src(self):
        ...
        self.assertEqual(accumulator.accumulated_masks[0]["src_text"], ['</s>', '_lui', '<unk>'])
        self.assertEqual(accumulator.accumulated_masks[1]["src_text"], ['</s>', '<unk>', '_lui', 'ava'])
    ...
    def test_update_heatmaps_single_batch(self):
        ...
        self.accumulator.accumulated_masks = {1: {
        ...
        self.assertEqual(self.accumulator.accumulated_masks[0]["fbank_heatmap"].shape, (3, 6, 10))
        self.assertEqual(self.accumulator.accumulated_masks[0]["tgt_embed_heatmap"].shape, (3, 3, 24))
        self.assertEqual(self.accumulator.accumulated_masks[1]["fbank_heatmap"].shape, (4, 7, 10))
        self.assertEqual(self.accumulator.accumulated_masks[1]["tgt_embed_heatmap"].shape, (4, 4, 24))
        # check if size of masks is preserved
        self.assertEqual(self.accumulator.accumulated_masks[0]["fbank_mask"].shape, (1, 6, 10))
        self.assertEqual(self.accumulator.accumulated_masks[0]["tgt_embed_mask"].shape, (1, 3, 24))
        self.assertEqual(self.accumulator.accumulated_masks[1]["fbank_mask"].shape, (1, 7, 10))
        self.assertEqual(self.accumulator.accumulated_masks[1]["tgt_embed_mask"].shape, (1, 4, 24))
        # check if existing entries are updated
        self.assertEqual(self.accumulator.accumulated_masks[1]["n_masks"], 3)
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[1]["fbank_heatmap"] == 1))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[1]["tgt_embed_heatmap"] == 1))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[1]["fbank_mask"] == 1))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[1]["tgt_embed_mask"] == 1))
        self.assertEqual(len(self.accumulator.accumulated_masks), 2)
        self.assertEqual(self.accumulator.accumulated_masks[0]["n_masks"], 2)
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[0]["fbank_heatmap"] == 2))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[0]["tgt_embed_heatmap"] == 2))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[0]["fbank_mask"] == 2))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[0]["tgt_embed_mask"] == 2))
        self.assertEqual(self.accumulator.accumulated_masks[0]["src_text"], "source text 1")
    ...
    def test_update_heatmaps_discrete_tgt(self):
        ...
        # check if size of heatmaps is preserved
        self.assertEqual(self.accumulator.accumulated_masks[0]["fbank_heatmap"].shape, (3, 6, 10))
        self.assertEqual(self.accumulator.accumulated_masks[0]["tgt_embed_heatmap"].shape, (3, 3, 1))
        self.assertEqual(self.accumulator.accumulated_masks[1]["fbank_heatmap"].shape, (4, 7, 10))
        self.assertEqual(self.accumulator.accumulated_masks[1]["tgt_embed_heatmap"].shape, (4, 4, 1))
        # check if size of masks is preserved
        self.assertEqual(self.accumulator.accumulated_masks[0]["fbank_mask"].shape, (1, 6, 10))
        self.assertEqual(self.accumulator.accumulated_masks[0]["tgt_embed_mask"].shape, (1, 3, 1))
        self.assertEqual(self.accumulator.accumulated_masks[1]["fbank_mask"].shape, (1, 7, 10))
        self.assertEqual(self.accumulator.accumulated_masks[1]["tgt_embed_mask"].shape, (1, 4, 1))
    ...
    def test_update_heatmaps_discrete_fbank(self):
        ...
        self.accumulator._update_heatmaps(
            ...
            torch.ones(3, 1, 4, 24))
        # check if size of heatmaps is preserved
        self.assertEqual(self.accumulator.accumulated_masks[0]["fbank_heatmap"].shape, (3, 6, 1))
        self.assertEqual(self.accumulator.accumulated_masks[0]["tgt_embed_heatmap"].shape, (3, 3, 24))
        self.assertEqual(self.accumulator.accumulated_masks[1]["fbank_heatmap"].shape, (4, 7, 1))
        self.assertEqual(self.accumulator.accumulated_masks[1]["tgt_embed_heatmap"].shape, (4, 4, 24))
        # check if size of masks is preserved
        self.assertEqual(self.accumulator.accumulated_masks[0]["fbank_mask"].shape, (1, 6, 1))
        self.assertEqual(self.accumulator.accumulated_masks[0]["tgt_embed_mask"].shape, (1, 3, 24))
        self.assertEqual(self.accumulator.accumulated_masks[1]["fbank_mask"].shape, (1, 7, 1))
        self.assertEqual(self.accumulator.accumulated_masks[1]["tgt_embed_mask"].shape, (1, 4, 24))
    ...
    def test_update_heatmaps_multiple_batch(self):
        ...
        self.assertEqual(self.accumulator.accumulated_masks[1]["fbank_heatmap"].shape, (4, 7, 10))
        self.assertEqual(self.accumulator.accumulated_masks[1]["tgt_embed_heatmap"].shape, (4, 4, 24))
        self.assertEqual(self.accumulator.accumulated_masks[2]["fbank_heatmap"].shape, (5, 9, 10))
        self.assertEqual(self.accumulator.accumulated_masks[2]["tgt_embed_heatmap"].shape, (5, 5, 24))
        # check if size of masks is preserved, with padding stripped correctly
        self.assertEqual(self.accumulator.accumulated_masks[0]["fbank_mask"].shape, (1, 6, 10))
        self.assertEqual(self.accumulator.accumulated_masks[0]["tgt_embed_mask"].shape, (1, 3, 24))
        self.assertEqual(self.accumulator.accumulated_masks[1]["fbank_mask"].shape, (1, 7, 10))
        self.assertEqual(self.accumulator.accumulated_masks[1]["tgt_embed_mask"].shape, (1, 4, 24))
        self.assertEqual(self.accumulator.accumulated_masks[2]["fbank_mask"].shape, (1, 9, 10))
        self.assertEqual(self.accumulator.accumulated_masks[2]["tgt_embed_mask"].shape, (1, 5, 24))
        # check if n. masks are correctly updated
        self.assertEqual(self.accumulator.accumulated_masks[0]["n_masks"], 2)
        self.assertEqual(self.accumulator.accumulated_masks[1]["n_masks"], 3)
        self.assertEqual(self.accumulator.accumulated_masks[2]["n_masks"], 2)
        # check if heatmaps and masks are correctly updated
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[0]["fbank_heatmap"] == 2))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[0]["tgt_embed_heatmap"] == 2))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[1]["fbank_heatmap"] == 3))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[1]["tgt_embed_heatmap"] == 3))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[2]["fbank_heatmap"] == 2))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[2]["tgt_embed_heatmap"] == 2))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[0]["fbank_mask"] == 2))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[0]["tgt_embed_mask"] == 2))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[1]["fbank_mask"] == 3))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[1]["tgt_embed_mask"] == 3))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[2]["fbank_mask"] == 2))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[2]["tgt_embed_mask"] == 2))
        # check src_texts
        self.assertEqual(self.accumulator.accumulated_masks[0]["src_text"], "source text 1")
        self.assertEqual(self.accumulator.accumulated_masks[1]["src_text"], "source text 2")
        self.assertEqual(self.accumulator.accumulated_masks[2]["src_text"], "source text 3")
        # check tgt_texts
        self.assertEqual(self.accumulator.accumulated_masks[0]["tgt_text"], ['</s>', '_lui', '<unk>'])
        self.assertEqual(self.accumulator.accumulated_masks[1]["tgt_text"], ['</s>', '<unk>', '_lui', 'ava'])
        self.assertEqual(self.accumulator.accumulated_masks[2]["tgt_text"], ['</s>', 'ava', '<unk>', '_lui', '_and'])
    ...
    def test_normalize(self):
        ...
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[0]["fbank_heatmap"] == 1))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[0]["tgt_embed_heatmap"] == 1))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[1]["fbank_heatmap"] == 1))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[1]["tgt_embed_heatmap"] == 1))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[2]["fbank_heatmap"] == 1))
        self.assertTrue(torch.all(self.accumulator.accumulated_masks[2]["tgt_embed_heatmap"] == 1))

    def test_normalize_division_by_zero(self):
        self.accumulator.accumulated_masks = {0: {}}
        self.accumulator.accumulated_masks[0]["fbank_heatmap"] = torch.ones(3, 4, 5)
        self.accumulator.accumulated_masks[0]["tgt_embed_heatmap"] = torch.ones(3, 3, 12)
        self.accumulator.accumulated_masks[0]["fbank_mask"] = torch.tensor(
            [[[2, 1, 0, 1, 2],
              [2, 2, 0, 1, 1],
              [2, 0, 1, 0, 1],
              [2, 2, 2, 1, 1]]])
        self.accumulator.accumulated_masks[0]["tgt_embed_mask"] = torch.tensor(
        ...
        self.assertTrue(torch.equal(
            self.accumulator.accumulated_masks[0]["fbank_heatmap"], expected_fbank_heatmap))
        self.assertTrue(torch.equal(
            self.accumulator.accumulated_masks[0]["tgt_embed_heatmap"], expected_tgt_heatmap))

    def test_call_method(self):
        file_path = self.accumulator.save_file_hdf5
        try:
